  ...
  <style>
    ...
    if($xxx == 1){
        ...
        if(isset($_POST['SubmitAns'])){
            $input = $_POST['ex1'];
            if($input == "title"){
                $output1 = "update score set soal1=1 where IDuser = '$username';";
    ...
    <div class="row align-items-center h-100" style="margin-top:10%;">
        <div class="col-lg-6 wow fadeInUp" style="margin-bottom: 0px;">
            <form autocomplete="off" id="w3-exerciseform" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="bg-dark" style="width:750px;; padding:10px;  border-radius:10px;">
                ...
                <h2>Atribute Exercise:</h2>
                <p>Add a "tooltip" to the paragraph below with the text "About W3Schools".</p>
                <div class="exerciseprecontainer">
                &lt;p <input class="float-center" name="ex1" maxlength="5" style="width: 54px;"  id="text-box1" value="<?php echo $input; ?>"
                             <?php if($xxx == 1) echo 'disabled' ?>>="About W3Schools"&gt;W3Schools is a web developer's site.&lt;/p&gt;
                </div>
                <br>
            ...
            <div class="d-flex justify-content-between" style="width:750px; margin-top:50px;">
                <div></div>
                <button class=" btn btn-dark"type="button" onclick="window.location.href='2.php'" style="width:120px;">Next »</button>
            </div>
      </div>
    </div>
        </div>
