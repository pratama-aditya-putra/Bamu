    ...
    if($xxx == 1){
        ...
        if(isset($_POST['SubmitAns'])){
            ...
            if(($input == "select")||($input == "SELECT")){
                $output1 = "update score set soal13=1 where IDuser = '$username';";
        ...
        <div class="col-lg-6 wow fadeInUp" style="margin-bottom: 0px;">
            <form autocomplete="off" id="w3-exerciseform" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="bg-dark" style="width:750px;; padding:10px;  border-radius:10px;">
                ...
                <div class="exercisewindow" style="background-color:white; color:black; padding:10px;">
                    <h2>Select Query Exercise:</h2>
                    <p>Insert the missing statement to get all the columns from the <code class="w3-codespan">Customers</code> table.</p>
                    <div class="exerciseprecontainer">
                    <pre><input name="ex1" maxlength="6" style="width: 64px;" id="text-box1" value="<?php echo $input; ?>" <?php if($xxx == 1) echo 'disabled' ?>> * FROM Customers;
                    </pre>
                    </div>
                    <br>
            ...
            <div class="d-flex justify-content-between" style="width:750px; margin-top:50px;">
                <button class=" btn btn-dark"type="button" onclick="window.location.href='12.php'" style="width:120px;">« Before</button>
                <button class=" btn btn-dark"type="button" onclick="window.location.href='14.php'" style="width:120px;">Next »</button>
            </div>
      </div>
    </div>
  </div>
